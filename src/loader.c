...
static unsigned long loadelf_anon(int fd, Elf_Ehdr *ehdr, Elf_Phdr *phdr)
{
	...
	{
		//一个程序头一个的读取
		// 获取内存地址映射范围 [min_va,max_va]

		if (iter->p_type != PT_LOAD)//只有PT_LOAD的断才会加载到内存中
		{
			continue;
		}
	...
	/* Check that we can hold the whole image. 
		检测是否可以在指定区域创建一块可用内存,如果成功创建就取消它....
	*/
	...
	for (iter = phdr; iter < &phdr[ehdr->e_phnum]; iter++) 
	{
		...
		//z_printf("base+%p(%p),",TRUNC_PG(iter->p_vaddr),iter->p_vaddr);
		//z_printf("size=%X(%X+%X)\n",ROUND_PG(iter->p_memsz + off),iter->p_memsz,off);
...
void z_entry(unsigned long *sp, void (*fini)(void))
{
	...
	file = argv[1];
	/*
	一般情况下: 第1次加载elf内存镜像, 第2次加载 ld.so 内存镜像
	*/
	for (i = 0;; i++, ehdr++) 
	{
		//第二次进入这个循环， 需要加载的是interpreter
		
		/* Open file, read and than check ELF header.*/
		if ((fd = z_open(file, O_RDONLY)) < 0)//打开文件
		{
			z_errx(1, "can't open %s", file);
		}
		if (z_read(fd, ehdr, sizeof(*ehdr)) != sizeof(*ehdr))//读取elf头
		{
			z_errx(1, "can't read ELF header %s", file);
		}
		if (!check_ehdr(ehdr))//查看是不是elf文件
		{
		...
		phdr = z_alloca(sz);
		if (z_lseek(fd, ehdr->e_phoff, SEEK_SET) < 0)//定位到程序头
		{
			z_errx(1, "can't lseek to program header %s", file);
		}
		if (z_read(fd, phdr, sz) != sz)//读取程序头
		{
			z_errx(1, "can't read program header %s", file);
		}
		/* Time to load ELF. */
		if ((base[i] = loadelf_anon(fd, ehdr, phdr)) == LOAD_ERR)//加载内存镜像
		{
			z_errx(1, "can't load ELF %s", file);
		}

		/* Set the entry point, if the file is dynamic than add bias. */
		entry[i] = ehdr->e_entry + (ehdr->e_type == ET_DYN ? base[i] : 0);//获取入口点
		/* The second round, we've loaded ELF interp. */
		if (file == elf_interp) //第一次循环中,file就在被赋值为了elf_interp
		{
			z_close(fd);
			break;
		}

		for (iter = phdr; iter < &phdr[ehdr->e_phnum]; iter++) //寻找interprter
		{
